...
Figma.Builder = function () {
    ...
    this.getHTMLTagName = function(tag){
        return 'div'
    }
    this.buildChildTagsHTMLString = function (tag, cssStyle, level) {
        if (tag.children.length > 0) {
            return '\n' + tag.children.map((child) => buildHTMLString(child, cssStyle, level + 1)).join('\n')
        }
        if (tag.isText) {
            return `${tag.textCharacters}`
        }
    ...
    this.buildHTMLString = function (tag, cssStyle, level) {
        if (!tag) {
            return ''
          }
        ...
        var hasChildren = tag.children.length > 0
        var className = this.getClassName(tag, cssStyle)
        var childTags = this.buildChildTagsHTMLString(tag, cssStyle, level)
    ...
    this.buildCode =  function (tag, codeType, cssStyle){
        ...
        if (codeType == 'react') {
            ...
            code += '<' + this.getHTMLTagName(tag) + '>' + this.buildHTMLString(tag, cssStyle, 0) + '</' + this.getHTMLTagName(tag) + '>';
        }
        return code;
    }
